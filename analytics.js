(() => {
    ...
    if (home) {
      ...
      if (infoList) {
        infoList.innerHTML = `
          ...
          <li><strong>Privacy details can be found <a href="/privacy.html" class="font-semibold text-sage-700 hover:text-sage-900 underline underline-offset-2">here</a>.</strong></li>
        `;
      }
  ...
  function installReportDesignUi() {
    ...
    if (!tensionsContainer) return;

    const tensionSection = tensionsContainer.closest('section');
    const tensionHeading = tensionSection?.querySelector('h3');

    // Keep the original report visual language. Only tensions change:
    // section heading outside, then full-width white cards stacked vertically.
    if (tensionSection) {
    ...
    tensionsContainer.className = 'space-y-3';

    const styleTensionCards = () => {
      Array.from(tensionsContainer.children).forEach(card => {
        card.className = 'bg-white p-5 rounded-xl border border-stone-200 space-y-2';

        const title = card.querySelector('h4');
        const copy = card.querySelector('p');

        if (title) {
          title.className = 'font-serif text-lg text-stone-900';
        }

        if (copy) {
          copy.className = 'text-sm text-stone-600 leading-relaxed';
        }
      });
    };
